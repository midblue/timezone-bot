...
module.exports = {
    ...
    async action({ msg, settings, match, typedUser, senderIsAdmin, }) {
        ...
        if (match[2].toLowerCase() === `me`)
            return me.action({ msg, settings, senderIsAdmin }, ActionProps);
        // escape hatch to role command
        if (match[2].substring(0, 3) === `<@&`)
            return role.action({ msg, settings, match }, ActionProps);
        // first, check for a timezone code
        const timezoneCodeLocationData = timezoneCodeToLocation(match[2]);
        if (timezoneCodeLocationData) {
            console.log(timezoneCodeLocationData);
            return (0, replyInChannel_1.send)(msg, `It's ${getLightEmoji(timezoneCodeLocationData.location)}${currentTimeAt(timezoneCodeLocationData.location, false, settings.format24)} in ${match[2]}. (${standardizeTimezoneName(timezoneCodeLocationData.timezoneName)})`, false, settings);
        }
        ...
        if (typedUser) {
            ...
            return (0, replyInChannel_1.send)(msg, `It's ${getLightEmoji(foundUser.location)}${currentTimeAt(foundUser.location, false, settings.format24)} for ${username}. (${standardizeTimezoneName(foundUser.timezoneName)})`, false, settings);
        }
