...
exports.default = {
    ...
    async action({ msg, settings, match }) {
        ...
        console.log(`${msg.guild
            ...
            : `Private Message`}${msg.guild ? ` (${msg.guild.id})` : ``} - Time at ${timeString} ${onlyHere ? `in #${msg.channel.name} ` : ``}(${msg.author.username})`);
...
Use \`${settings.prefix}at <time> <location/user>\` to see other users' times at a certain time.`;
        ...
        const entries = {};
        const guildMembers = (await (0, commonFunctions_1.getGuildMembers)({ msg })).filter((guildMember) => onlyHere
