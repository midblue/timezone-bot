module.exports = function (limit, name) {
  return {
    ...
    set: function (key, value) {
      ...
      if (!this.limit || Object.keys(this.memos).length <= this.limit) return
    ...
    get: function (key) {
      const found = this.memos[key] ? this.memos[key].value : undefined
      if (found) delete found.memoAddedTime
      return found
    },
    updateProp: function (key, prop, newData) {
      const found = this.memos[key] ? this.memos[key].value : undefined
      if (found && typeof found === 'object') found[prop] = newData
      return found
    },
    delete: function (key) {
