...
\`!help\` to show this message.`);
  ...
  async at(msg, atsInMessage, senderTimezoneOffset) {
  ...
  me(msg, senderUsername) {
    ...
    if (!userInfo)
      return msg.channel.send(
        `No timezone has been set for ${senderUsername}.`
      );
    msg.channel.send(
      `\`It's ${format.currentTimeAt(userInfo.location)} for ${
        userInfo.username
  ...
  async timeAt(msg) {
    ...
    if (foundUsername) {
      ...
      if (!foundTimezone)
        return msg.channel.send(
          `No timezone listed for user ${foundUsername}.`
        );
      msg.channel.send(
